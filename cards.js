// You can add new scenarios, but make sure that there is exactly...
// * 1 monsterCard per scenario
// * 3 playerCards per scenario

let scenarios = [
  {
    monsterCard : {
      description : "A prehistoric monster burrows up from the earths center and tries to enslave mankind",
      power : 6,
    },
    playerCards : [
      {
        description : "Put it back into a cryogenic sleep",
        power : 8,
      },
      {
        description : "Attack it with nuclear weapons",
        power : 3,
      },
      {
        description : "Become friends with it",
        power : 5,
      }
    ]
  },
  {
    monsterCard : {
      description : "A strange thing begins to happen, people start developing second shadows",
      power : 7,
    },
    playerCards : [
      {
        description : "Get as close as you can to the second shadow",
        power : 2,
      },
      {
        description : "Run away as fast as you can",
        power : 9,
      },
      {
        description : "Make the person stand as still as possible, preferably in a dark room",
        power : 2,
      }
    ]
  },
  {
    monsterCard : {
      description : "A strange alien being has been seen from the corner of your eye, with shock you realise you forgot they exist!",
      power : 4,
    },
    playerCards : [
      {
        description : "Make a mark on your skin to remember you have seen it",
        power : 3,
      },
      {
        description : "Turn away as quickly as possible",
        power : 2,
      },
      {
        description : "Transmit a recording of one of the aliens telling people to attack them on sight",
        power : 7,
      }
    ]
  },
  {
    monsterCard : {
      description : "Half robot half human beings appear all over the planet and begin to delete humans",
      power : 7,
    },
    playerCards : [
      {
        description : "Use a neural inhibitor to make them feel emotions",
        power : 9,
      },
      {
        description : "Shoot them with their own guns",
        power : 6,
      },
      {
        description : "Bring a stronger race of robot aliens to fight them off ",
        power : 8,
      }
    ]
  },
  {
    monsterCard : {
      description : "A race of rhino people teleport the building you're in onto the moon",
      power : 5,
    },
    playerCards : [
      {
        description : "Rally round everybody to make sure they're all in the safest place",
        power : 2,
      },
      {
        description : "Help them capture the escaped convict they hunt ",
        power : 7,
      },
      {
        description : "Tell them that you are the person they look for, and to let everyone else go",
        power : 4,
      }
    ]
  }, 

  {
    monsterCard : {
      description : "Reports of statues moving in the local cemetary gets your attention",
      power : 6,
    },
    playerCards : [
      {
        description : "Don't blink!",
        power : 6,
      },
      {
        description : "Trick the statues into looking directly at eachother",
        power : 8,
      },
      {
        description : "Sonic them with your screwdriver!",
        power : 2,
      }
    ]
  }, 

  {
    monsterCard : {
      description : "The prime minister captures you and puts you in a cage",
      power : 7,
    },
    playerCards : [
      {
        description : "Trust your companion to rescue you by telling stories, no matter how long it takes!",
        power : 8,
      },
      {
        description : "Take advantage of the prime ministers insanity",
        power : 4,
      },
      {
        description : "Use the death particle to wipe out all life on the planet",
        power : 2,
      }
    ]
  }, 
  {
    monsterCard : {
      description : "A race of shape shifters take up residence on Earth",
      power : 8,
    },
    playerCards : [
      {
        description : "Poison their milk supply to put them to sleep",
        power : 8,
      },
      {
        description : "Detonate a nuclear weapon beneath London to destroy everyone!",
        power : 3,
      },
      {
        description : "Draw up a peace treaty to allow them to live alongside humans",
        power : 9,
      }
      ] 
    },
      {
      monsterCard : {
        description : "A robot hybrid creature appears, there seems to be only one of them!",
        power : 8,
        },
      playerCards : [
        {
          description : "Get your companion to open the TARDIS console and absorb its essence before more appear",
          power : 9,
        },
        {
          description : "Attack it with as many weapons as possible",
          power : 3,
        },
        {
          description : "Get Donna to shout at them",
          power : 2,
        }
      ]
      },
      {
      monsterCard : {
        description : "You encounter a crack in the wall of your new friends bedroom",
        power : 5,
      },
      playerCards : [
        {
          description : "Open the crack wider to see whats inside",
          power : 7,
        },
        {
          description : "Ignore it, its just a crack",
          power : 4,
        },
        {
          description : "Trap it in a timelock and think about it again later",
          power : 2,
        }
      ]
    },
    {
      monsterCard : {
        description : "Humans all over the planet begin to head to the roof, it looks like they're going to jump!",
        power : 5,
      },
      playerCards : [
        {
          description : "Press the big red button",
          power : 8,
        },
        {
          description : "Sword fight the alien species!",
          power : 6,
        },
        {
          description : "Get TORCHWOOD to do a temporal shift",
          power : 2,
        }
      ]
    }, 

    {
      monsterCard : {
        description : "A gas mask wearing child haunts people in Blitzed London, asking for his Mummy",
        power : 6,
      },
      playerCards : [
        {
          description : "Find a way to trap the child",
          power : 4,
        },
        {
          description : "Give the child a cuddle!",
          power : 3,
        },
        {
          description : "Find his Mummy",
          power : 8,
        }
      ]
    }, 

    {
      monsterCard : {
        description : "You are trapped in a castle, being slowly chased by a monster, every time you die you start from the beginning again",
        power : 4,
      },
      playerCards : [
        {
          description : "Use your sonic screwdriver on the doors",
          power : 3,
        },
        {
          description : "Run from the monster!",
          power : 2,
        },
        {
          description : "Find your TARDIS and punch the diamond it is encased in for thousands of years to release it",
          power : 8,
        }
      ]
    },
  ];